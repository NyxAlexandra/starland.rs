...
#[cfg(feature = "egl")]
impl DmabufHandler for StarlandState<UdevData> {
...
#[cfg(feature = "egl")]
delegate_dmabuf!(StarlandState<UdevData>);
...
pub fn run_udev(log: Logger) {
     ...
     */
    let primary_gpu = if let Ok(var) = std::env::var("STARLAND_DRM_DEVICE") {
    ...
    let dmabuf_state = {
        ...
        if renderer.bind_wl_display(&display.handle()).is_ok() {
            ...
            let global = state.create_global::<StarlandState<UdevData>, _>(
                &display.handle(),
                dmabuf_formats,
                log.clone(),
            );
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<StarlandState<UdevBackend>>(global);
        }
    }
}
...
fn scan_connectors(
    ...
    gbm: &Rc<RefCell<GbmDevice<SessionFd>>>,
    display: &mut Display<StarlandState<UdevData>>,
    ...
    for connector_info in connector_infos {
        ...
        for crtc in crtcs {
            ...
            let global = output.create_global::<StarlandState<UdevData>>(&display.handle());
            let position = (
                space
                    .outputs()
